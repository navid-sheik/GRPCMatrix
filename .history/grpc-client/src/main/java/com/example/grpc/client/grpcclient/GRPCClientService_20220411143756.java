		...
		for(int rowA=0;rowA<matrixA.length;rowA++){    
			for(int rowB=0;rowB<matrixB.length;rowB++){    
				...
				{  
					...
					MatrixReply  multValue = stub.multiplyBlock(MatrixRequest.newBuilder().setA().setB().build());
					//Add the value to existing c[i][j] - initiallly 0
					MatrixReply  addValue = stub.addBlock(MatrixRequest.newBuilder().setA().setB().build());
					//Update the value
					matrixC[rowA][rowB] = addValue;
