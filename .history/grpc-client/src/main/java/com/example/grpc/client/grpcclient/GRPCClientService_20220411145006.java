		...
		for(int rowA=0;rowA<matrixA.length;rowA++){    
			for(int rowB=0;rowB<matrixB.length;rowB++){    
				...
				{  
					...
					MatrixReply  multValue = stub.multiplyBlock(MatrixRequest.newBuilder().setA(matrixA[rowA][k]).setB(matrixB[k][rowB]).build());
					//Add the value to existing c[i][j] - initiallly 0
					MatrixReply  addValue = stub.addBlock(MatrixRequest.newBuilder().setA(matrixC[rowA][rowB]).setB(multValue.getC()).build());
					//Update the value
