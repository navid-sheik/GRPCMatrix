...
public class GRPCClientService {
	...
	private static  void clientOperation(int[][] matrixA, int[][] matrixB){
		...
		MatrixServiceGrpc.MatrixServiceBlockingStub stubToUse = stub1;
		for(int rowA=0;rowA<matrixA.length;rowA++){    
			for(int rowB=0;rowB<matrixB.length;rowB++){    
				...
				{  
					...
					MatrixReply  multValue = stubToUse.multiplyBlock(MatrixRequest.newBuilder()
												.setA(matrixA[rowA][k])
												.setB(matrixB[k][rowB])
												.build());
					//Add the value to existing c[i][j] - initiallly 0
					MatrixReply  addValue = stubToUse.addBlock(MatrixRequest.newBuilder()
												.setA(multValue.getC())
